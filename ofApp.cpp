...
void ofApp::setup() {
	...
	for (int i = 0; i < this->number_of_targets; i++) {
		...
		this->circles.back().get()->setup(this->box2d.getWorld(), ofRandom(ofGetWidth()), ofRandom(ofGetHeight()), 10);
...
void ofApp::update() {
	...
	float direction = ofGetFrameNum() % 60 == 0 ? 1.0 : -0.25;
	//float direction = ofGetKeyPressed() ? 2.0 : -0.5;
